...
class Generator(nn.Module):
    def __init__(self, latent_dim=100):
        ...
        self.forward_pass = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Dropout(0.5),  # add 3x dropout after intermediate hidden layers
            nn.Linear(128, 256),
            nn.BatchNorm1d(256),
            nn.LeakyReLU(0.2),
            nn.Dropout(0.5),  # add 3x dropout after intermediate hidden layers
            ...
            nn.LeakyReLU(0.2),
            nn.Linear(1024, 784),
            nn.Tanh()  # because the images are normalized between -1 and 1
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.forward_pass = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Linear(256, 1),
            nn.Sigmoid()  # because we want probability that it comes from the real data set
        )
...
def sample(generator, device):
    ...
    Gz = generator(z)
    save_image(Gz,
               'images/{}-2-6.png'.format(str(int(time.time()))),  # for question 2.6
...
def train(dataloader, discriminator, generator, optimizer_G, optimizer_D, device):
    ...
    #  track duration of epochs-----------------------
    track_time = time.time()
    # ------------------------------------------------

    for epoch in range(args.n_epochs):
        # time tracking of how long an epoch takes ---
        print("--------------------")
        ...
        for i, (imgs, _) in enumerate(dataloader):
            ...
            # Train Discriminator without any fancy tricks
            # -------------------
            ...
            # Train Generator without any fancy tricks except for dropout layers in the model
            # ---------------
            z = torch.randn((x.shape[0], args.latent_dim), device=device)
    ...
    dataloader = torch.utils.data.DataLoader(
        datasets.MNIST('./data/mnist', train=True, download=True,
                       transform=transforms.Compose([
                           transforms.ToTensor(),
                           #  TODO @TA PLEASE CHANGE THIS IN SOURCE CODE FOR NEXT YEAR
...
def interpolate():
    ...
    I manually looked for nice images and the z-distributino they came from
    the 'one' and 'four' are the results of that searc

    """
    generator = Generator()
    generator.load_state_dict(torch.load('../gan_images/mnist_generator.pt', map_location='cpu'))

    # this was used for creating images and searching for good ones
    for i in range(10):
        a = torch.randn(1, 100)
        print(a)
        generator.eval()
    ...
    diff = four - one  # get the euclidian distance
    steps = 8  # take 8 steps
    step = diff / steps  # take 8 equal steps
    z_interpolation = torch.stack([one + i * step for i in range(steps + 1)])  # build the intermediate images
    gen_interpolation = generator(z_interpolation)  # run them through the generator
    gen_interpolation = gen_interpolation.reshape(-1, 1, 28, 28)  # make them into images
    save_image(gen_interpolation,  # save the images!
